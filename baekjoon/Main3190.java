...
public class Main3190 {
	...
	public static void main(String[] args) throws Exception{
		...
		//입력
		N=Integer.parseInt(br.readLine().trim());	//맵 크기
		K=Integer.parseInt(br.readLine().trim());	//사과 개수
		map=new int[N][N];
		String[] s=new String[K];
		for(int i=0;i<K;i++) {
			s=br.readLine().split(" ");
			map[Integer.parseInt(s[0])-1][Integer.parseInt(s[1])-1]=-1;	//사과 행 좌표
		}
		...
		for(int i=0;i<L;i++) {
			s=br.readLine().split(" ");
			snakeTime[i]=Integer.parseInt(s[0]);	//뱀 방향전환 시간
			snakeDir[i]=s[1].charAt(0);				//뱀 방향 어디?
		}
		...
		if(map[newX][newY]==-1) {		//사과 먹음
			...
			for(int t=0;t<L;t++) {	//방향돌리는 시간 확인
				if(cnt==snakeTime[t]) {		//시간찾음!!
					if(snakeDir[t]=='D') {
						if(dirSnake!=4) dirSnake++;
						else dirSnake=1;
						solve(newX,newY,dirSnake);
						return;
					}
					else {
						if(dirSnake!=1) dirSnake--;
						else dirSnake=4;
						solve(newX,newY,dirSnake);
						return;
					}
